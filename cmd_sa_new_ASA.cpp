...
vector<string> new_cmd_simulated_annealing(double& low_effort_best_cost, double& best_cost, bool buf_flag, vector<string>& init_actions, map<string, pair<string, double>>& gate_cost_dic){
    ...
    float r = 0.95;
    int step_num = 0;
    double max_delta_cost = 0;
    double cur_delta_cost;
    vector<double> five_recent_cost;
    while(T > T_low){
        temp_action = cur_action;
        pRecord = Abc_NtkDup(Abc_FrameReadNtk(abcMgr->get_Abc_Frame_t()));

        double window_total_cost = 0;
        for (int i = 0; i < five_recent_cost.size(); i++) {
            window_total_cost += five_recent_cost[i];
        }
        double avg_cost = window_total_cost / 5;
        int round;
        double adaptive_factor = orig_cost / avg_cost;
        cout << "adative: " << adaptive_factor << endl;
        if (step_num < 5) {
            round = 3;
        }
        else if (adaptive_factor < 0.95) {
            round = 1;
        }
        else if (adaptive_factor < 0.983) {
            round = 2;
        }
        else if (adaptive_factor < 1.017) {
            round = 3;
        }
        else {
            round = 4;
        ...
        if (step_num < 5) {
            five_recent_cost.push_back(after_cost);
        }
        else {
            five_recent_cost[step_num % 5] = after_cost;
        }
        cur_delta_cost = orig_cost - after_cost;
        if (cur_delta_cost > 0 && max_delta_cost == 0) {
            max_delta_cost = cur_delta_cost;
        }
        step_num++;
        ...
        if (max_delta_cost == 0 || cur_delta_cost < 0) {
            r = 0.95;
        }
        else if (cur_delta_cost > max_delta_cost) {
            r = 0.75;
            max_delta_cost = cur_delta_cost;
        }
        else {
            r = 0.95 - (cur_delta_cost / max_delta_cost) * 0.2;
        }
        cout << "r: " << r << endl;
        T = r*T;
    }
