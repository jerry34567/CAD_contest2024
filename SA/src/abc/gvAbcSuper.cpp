...
void Abc_Super_Precompute()
{
    ...
    Vec_Str_t * vStr;
    Mio_Library_t* pLibGen = Mio_LibraryRead("contest.genlib", NULL, NULL, 0);
    float DelayLimit;
    float AreaLimit;
    int fSkipInvs;
    int fWriteOldFormat; 
    ...
    vStr = Abc_Super_PrecomputeStr( pLibGen, nVarsMax, nLevels, nGatesMax, DelayLimit, AreaLimit, TimeLimit, fSkipInvs, fVerbose );

    FILE * pFile = fopen( "contest.super", "wb" );
    if ( vStr ) 
    {
...
string super_to_base_rec(string name0, string name1, string name2, string name3, string name4, string output, TreeStr* tree_string) {
    string left;
    string right;
    string wire_l = "";
    string wire_r = "";
    string ans;
    if (!tree_string->get_left()->Is_leaf()) {
        wire_l = "wire_" + to_string(abcSuperMgr->get_wire_counter());
        abcSuperMgr->add_wire_counter();
        left = super_to_base_rec(name0, name1, name2, name3, name4, wire_l, tree_string->get_left());
    }
    else {
        if (tree_string->get_left()->get_content() == "0") {
            left = ".A(" + name0 + ")";
        }
        else if (tree_string->get_left()->get_content() == "1") {
            left = ".A(" + name1 + ")";
        }
        else if (tree_string->get_left()->get_content() == "2") {
            left = ".A(" + name2 + ")";
        }
        else if (tree_string->get_left()->get_content() == "3") {
            left = ".A(" + name3 + ")";
        }
        else if (tree_string->get_left()->get_content() == "4") {
            left = ".A(" + name4 + ")";
        }
    }

    if (!tree_string->get_right()->Is_leaf()) {
        wire_r = "wire_" + to_string(abcSuperMgr->get_wire_counter());
        abcSuperMgr->add_wire_counter();
        right = super_to_base_rec(name0, name1, name2, name3, name4, wire_r, tree_string->get_right());
    }
    else {
        if (tree_string->get_right()->get_content() == "0") {
            right = ".B(" + name0 + ")";
        }
        else if (tree_string->get_right()->get_content() == "1") {
            right = ".B(" + name1 + ")";
        }
        else if (tree_string->get_right()->get_content() == "2") {
            right = ".B(" + name2 + ")";
        }
        else if (tree_string->get_right()->get_content() == "3") {
            right = ".B(" + name3 + ")";
        }
        else if (tree_string->get_right()->get_content() == "4") {
            right = ".B(" + name4 + ")";
        }
    }

    if (tree_string->get_right()->Is_leaf() && tree_string->get_left()->Is_leaf()) {
        ans += tree_string->get_content() + "(" +  left + ", " + right + ", " + ".Y(" + output + "));\n";
    }
    else if (!tree_string->get_left()->Is_leaf() && tree_string->get_right()->Is_leaf()) {
        ans += left;
        ans += tree_string->get_content() + "(" + ".A(" + wire_l + ")" + ", " + right + ", " + ".Y(" + output + "));\n";
    }
    else if (tree_string->get_left()->Is_leaf() && !tree_string->get_right()->Is_leaf()) {
        ans += right;
        ans += tree_string->get_content() + "(" + left + ", " + ".B(" + wire_r + ")" + ", " + ".Y(" + output + "));\n";
    }
    else {
        ans += left;
        ans += right;
        ans += tree_string->get_content() + "(" + ".A(" + wire_l + ")" + ", " + ".B(" + wire_r + ")" + ", " + ".Y(" + output + "));\n";
    }
    return ans;
}

double get_super_cost(TreeStr* tree_string) {
    ofstream write_file("temp_file.v");
    write_file << "module " << costMgr->get_module_name() << " (a, b, c, d, e, o);\n";
    write_file << "input a, b, c, d, e;\n";
    write_file << "output o;\n";
    
    abcSuperMgr->reset_wire_counter();
    string temp = super_to_base_rec("a", "b", "c", "d", "e", "o", tree_string);
    write_file << temp;
    write_file << "endmodule\n";
    write_file.close();

    string args = "-library " + costMgr->get_lib() + " -netlist temp_file.v -output temp.out";
    string output = exec(costMgr->get_cost_exe(), args);
...
***********************************************************************/
string Abc_Super_Write_Function_rec( Super_Gate_t * pGate, AbcSuperGate * pSGate) 
{
    string function = "";
    if ( pGate->pRoot == NULL)
    {
        function += ('A' + pGate->Number);
        return function;
    }
    string type = costMgr->get_type(Mio_GateReadName(pGate->pRoot));
    ...
    if (type == "nand" || type == "and") {
        function += "(";
        function += Abc_Super_Write_Function_rec(pGate->pFanins[0], pSGate);
        function += ")";
        function += "*";
        function += "(";
        function += Abc_Super_Write_Function_rec(pGate->pFanins[1], pSGate);
        function += ")";
    }
    else if (type == "nor" || type == "or") {
        function += "(";
        function += Abc_Super_Write_Function_rec(pGate->pFanins[0], pSGate);
        function += ")";
        function += "+";
...
void Super_WriteLibraryGateName_rec( Super_Gate_t * pGate, char * pBuffer, AbcSuperGate * pSGate, TreeStr* tree_string )
{
    char Buffer[10];
    int i;
    tree_string->set_left(new TreeStr);
    tree_string->set_right(new TreeStr);
    if ( pGate->pRoot == NULL )
    {
        sprintf( Buffer, "%c", 'a' + pGate->Number );
        strcat( pBuffer, Buffer );
        tree_string->set_content(to_string(pGate->Number));
        tree_string->set_leaf(true);
        return;
    }
    strcat( pBuffer, Mio_GateReadName(pGate->pRoot) );
    tree_string->set_content(Mio_GateReadName(pGate->pRoot));
    strcat( pBuffer, "(" );
    for ( i = 0; i < (int)pGate->nFanins; i++ )
    {
        if ( i ) {
            strcat( pBuffer, "," );
            Super_WriteLibraryGateName_rec( pGate->pFanins[i], pBuffer, pSGate, tree_string->get_right() );
        }
        else {
            Super_WriteLibraryGateName_rec( pGate->pFanins[i], pBuffer, pSGate, tree_string->get_left() );
        }
    }
    strcat( pBuffer, ")" );
}
char * Super_WriteLibraryGateName( Super_Gate_t * pGate , AbcSuperGate * pSGate)
{
    static char Buffer[2000];
    Buffer[0] = 0;
    TreeStr* tree_string = new TreeStr;
    Super_WriteLibraryGateName_rec( pGate, Buffer, pSGate, tree_string);
    string function = Abc_Super_Write_Function_rec(pGate, pSGate);
    pSGate->set_function(function);
    pSGate->set_tree_function(tree_string);
    pSGate->set_area(get_super_cost(tree_string));
    return Buffer;
}
void Super_WriteLibraryGate( FILE * pFile, Super_Man_t * pMan, Super_Gate_t * pGate, int Num, AbcSuperGate * pSGate )
{
    int i;
    ...
    fprintf( pFile, "   " );
    fprintf( pFile, "%s", Super_WriteLibraryGateName(pGate, pSGate) );      // the symbolic expression
    fprintf( pFile, "\n" );
}
...
***********************************************************************/
void Super_WriteLibraryTreeStr_rec( Vec_Str_t * vStr, Super_Man_t * pMan, Super_Gate_t * pSuper, int * pCounter, AbcSuperGate* pSGate)
{
    int nFanins, i;
    // skip an elementary variable and a gate that was already written
    if ( pSuper->fVar || pSuper->Number > 0 )
    ...
    nFanins = Mio_GateReadPinNum(pSuper->pRoot);
    for ( i = 0; i < nFanins; i++ )
        Super_WriteLibraryTreeStr_rec( vStr, pMan, pSuper->pFanins[i], pCounter, pSGate );
    // finally write the gate
    pSuper->Number = (*pCounter)++;
//    fprintf( pFile, "%s", pSuper->fSuper? "* " : "" );
//    fprintf( pFile, "%s", Mio_GateReadName(pSuper->pRoot) );
...
//    fprintf( pFile, "\n" );
    if (pSuper->fSuper) {
        Vec_StrPrintStr( vStr, "    # ");
        Vec_StrPrintStr( vStr, Super_WriteLibraryGateName( pSuper, pSGate ));
        Vec_StrPrintStr( vStr, (to_string(pSuper->Number)).c_str() );
        abcSuperMgr->add_numId_to_cost(pSuper->Number, pSGate->get_area());
    }
    Vec_StrPrintStr( vStr, "\n" );
}
Vec_Str_t * Super_WriteLibraryTreeStr( Super_Man_t * pMan )
{
    ...
    Counter = pMan->nVarsMax;
    Super_ManForEachGate( pMan->pGates, pMan->nGates, i, pSuper ) {
        AbcSuperGate* pSGate = new AbcSuperGate;
        Super_WriteLibraryTreeStr_rec( vStr, pMan, pSuper, &Counter, pSGate );
        if (pSGate->get_function().empty()) {
            delete pSGate;
            continue;
        }
        if (pSuper->fSuper) {
            abcSuperMgr->add_super_gate(("super" + to_string(i)), pSGate);
            // cout << pSGate->get_function() << endl;
        }
    }
    Vec_StrPush( vStr, 0 );
    // write the number of lines
...
void Abc_replace_super(string file_name) {
    ...
    abcSuperMgr->reset_wire_counter();
    ofstream tempFile("temp1.v");
    string line;
    regex pattern(R"(super\d+.*?)");
    vector<std::string> pins = {".A", ".B", ".C", ".D", ".E", ".Y"};
    if (file.is_open()) {
        while (getline(file, line)) {
            ...
            if (std::regex_search(line, matches, pattern)) {
                ...
                for (const auto& pin : pins) {
                    std::regex pinPattern(pin + R"(\((.*?)\))"); // 建立針對每個引腳的正則表達式
                    std::smatch pinMatches;
                    if (std::regex_search(line, pinMatches, pinPattern)) {
                        // std::cout << pin << ": " << pinMatches.str(1) << std::endl; // 輸出引腳內容
                        if(pin == ".A"){
                            name0 = pinMatches.str(1);
                        }
                        else if (pin == ".B") {
                            name1 = pinMatches.str(1);
                        }
                        else if (pin == ".C") {
                            name2 = pinMatches.str(1);
                        }
                        else if (pin == ".D") {
                            name3 = pinMatches.str(1);
                        }
                        else if (pin == ".E") {
                            name4 = pinMatches.str(1);
                        }
                        else if (pin == ".Y") {
                            output = pinMatches.str(1);
                        }
                    }
                }
                tempFile << super_to_base_rec(name0, name1, name2, name3, name4, output, abcSuperMgr->get_super(matches.str(0))->get_tree_function());
            }
            else {
                tempFile << line << endl;
            }
    ...
    ifstream temp_file("temp1.v");
    ofstream revise_file(file_name);
    bool foundWire = false; // 用於標記是否已經找到 "wire"

    if (temp_file.is_open() && revise_file.is_open()) {
        while (getline(temp_file, line)) {
            if (!foundWire && line.find("wire") != std::string::npos) {
                size_t pos = line.find("wire");
                if (pos != std::string::npos) {
                    pos += 5;
                    for (int i = 0; i < abcSuperMgr->get_wire_counter(); i++) {
                        line.insert(pos, "wire_" + to_string(i) + ", ");
                        pos += 7 + to_string(i).size();
                    }
                    foundWire = true; // 標記已經找到 "wire"
                }
            }
            revise_file << line << std::endl; // 將當前行寫入臨時檔案
        }
        temp_file.close();
        revise_file.close();
    }
...
void reset_super_area() {
    ...
    Map_Super_t* pGate;
    for (int i = 0; i < pLib->nSupersReal; i++) {
        pGate = pLib->ppSupers[i];
        double cost = abcSuperMgr->get_super_cost(pGate->Num);
        if (cost != 0)
            pGate->Area = cost;
    }   
}
